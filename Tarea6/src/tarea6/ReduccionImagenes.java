...
public class ReduccionImagenes { 
    ...
    /**
     * Método que aplica el filtro mosaico a la imagen recibidA
     * @param imagen la imagen recibida
     * @param x el ancho de los cuadros en el filtro mosaico
     * @param y el alto de los cuadros en el filtro mosaico
     * @return la imagen con el filtro mosaico aplicado
     */
    ...
    /**
     * Método que reduce una imagen dada la resolución 16x16,24x24,48x48, 64x64
     * o los parámetros que se den ya sea en x y y.
     * @param imagen la imagena reducir
     * @param x el ancho de la resolución
     * @param y el alto de la resolución
     * @return la imagen reducida
     */
    public BufferedImage reduceImagen(BufferedImage imagen, int x, int y){
        BufferedImage m = mosaico(imagen, x, y);
        // Calculamos la cantidad de cuadros que habrá en el ancho y alto
        int columnas = (int)(m.getWidth()/x);
        int filas = (int)(m.getHeight()/y);
        // Creamos una nueva imagen
        BufferedImage reducida = new BufferedImage(x, y, BufferedImage.TYPE_INT_RGB);
        int cy = 0;
        // Vamos tomando un color de cada cuadro del filtro mosaico
        for(int i = 0; i < m.getHeight(); i+=filas){
            int cx = 0;
            for(int j = 0; j < m.getWidth(); j+=columnas){
                int color = m.getRGB(j, i);
                if(cx < x && cy < y){
                    reducida.setRGB(cx, cy, color);
                }
                cx++;
            }
            cy++;
